...
def main(test_dir, train_dir, test_labels, train_labels):
    ...
    features, ch2, classifier = train(train_files, y_train)
    test(test_files, y_test, classifier, features, ch2)
    """
...
def train(documents, labels):
    ...
    design_matrix = ch2.fit_transform(design_matrix, labels)
    retained_feature_names = [feature_names[i] for i in ch2.get_support(indices=True)]
    print retained_feature_names
    classifier = MultinomialNB()#BernoulliNB() #LogisticRegression()
    classifier.fit(design_matrix, labels)
    return feature_names, ch2, classifier

def test(documents, labels, classifier, feature_names, ch2):
    print 'Testing'
    global my_tokenizer
    inverted_index = indexDocuments(documents)
    design_matrix = getDesignMatrix(inverted_index, feature_names, documents)
    design_matrix = ch2.transform(design_matrix)
